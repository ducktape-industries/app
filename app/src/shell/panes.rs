...
pub(super) struct MountedPane {
    ...
    pub(super) view: Entity<crate::module_view::NativeModuleView>,
...
impl DesktopWindow {
    pub(super) fn focused_view(&self) -> Option<Entity<crate::module_view::NativeModuleView>> {
        self.layout
            .panes
            ...
            if mounted.route.is_none() {
                ...
                mounted.route = Some(cx.subscribe(&mounted.view, move |_, _, event, cx| {
                    if event.kind == "context" {
                        *context.borrow_mut() = crate::module_view::event_text(event, "text");
                        cx.notify();
                    }
