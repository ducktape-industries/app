        ...
        RECORDED.with_borrow_mut(|events| {
            if let Some(events) = events {
                ...
                if matches!(
                    ...
                    while matches!(
                        events.last(),
                        Some((module, wire::Event::Mouse {
                            event: wire::mouse::Event::CursorMoved { .. },
                            ..
                        })) if *module == guest.module
                    ) {
                        events.pop();
                    }
                }
                events.push((guest.module, observed));
            }
        });
    }
    admitted
...
#[cfg(test)]
thread_local! {
    static RECORDED: RefCell<Option<Vec<(&'static str, wire::Event)>>> =
...
pub(super) fn recorded_inputs() -> Vec<wire::Event> {
    recorded_inputs_by_guest()
        .into_iter()
        .map(|(_, event)| event)
        .collect()
}
/// Every admitted event with the module of the guest that took it.
#[cfg(test)]
pub(super) fn recorded_inputs_by_guest() -> Vec<(&'static str, wire::Event)> {
