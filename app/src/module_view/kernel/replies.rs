...
pub(in crate::module_view) struct Replies {
...
impl Replies {
    ...
    pub(in crate::module_view) fn changes(&self) -> tokio::sync::watch::Receiver<()> {
        self.changed.subscribe()
    }

    pub(in crate::module_view) fn drain_into(
        &self,
    ...
    pub(in crate::module_view) fn fault(&self) -> Option<String> {
    ...
    #[cfg(test)]
    pub(in crate::module_view) fn any_in_flight(&self) -> bool {
    ...
    pub(in crate::module_view) fn answer_owed(&self) -> bool {
        let events = self.events.lock().expect("kernel replies");
