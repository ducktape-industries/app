...
pub(super) fn content_dimensions(
    ...
    let from_native = |length: Option<gpui_kit::Length>| {
        length.and_then(|length| match length {
    ...
    match node {
        wire::Node::Space { width, height } | wire::Node::Scroll { width, height, .. } => {
            (*width, *height)
        }
        ...
        wire::Node::Container {
            ...
            let native = (
                from_native(style.size.width),
                from_native(style.size.height),
            );
