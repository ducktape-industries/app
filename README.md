...
`python3 ops/qa/walk.py <scenario.json> --out <dir> [--param k=v …] [--keep-going]` walks the app through the door. Plain Python 3 stdlib. Exit 0 PASS, 1 FAIL, 2 FAIL-UNJUDGED, 3 a bad scenario or command line.
...
- Scenario: `{name, params: {name: doc}, rig: {display, env, secrets}, steps: [{say, expect?, kind?, …}]}`. `{param:k}`, `{rig:dir|home|config|data|state|run|ducktape_home}` and `{secret:name}` fill argv and text. Kinds:
  ...
  - `wait` (`wait: {role, name, state, in, gone}`, `deadline_ms`), `shell` (fixed `argv`, no shell; `expect_exit`, `expect_output` regex; `background` keeps it running), `launch` (`argv`, waits for the door), `stop`: no model call.
